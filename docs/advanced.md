...
- **样本**来自成功的单条调用，取 `服务耗时 = 端到端耗时 − 客户端本地排队`（semaphore 与 QPS 漏桶造成的等待不算在 endpoint 头上）。两类结果不计入，否则 endpoint 会显得比实际快而吸走流量：
  - **失败**：往往返回得更快，计入会让坏 endpoint 显得更优；处理失败是健康检查（`failure_threshold`）的职责
  - **缓存命中**：耗时是微秒级的本地读取，而且缓存跨 endpoint 共享，命中与否和是哪个 endpoint 无关
