...
- **单条调用**（`chat_completions`）：延迟感知 + 容量感知，两者分工明确——

  - 容量：饱和（`in_flight >= concurrency_limit`）的 endpoint 直接出局，全部饱和时退回轮询（此时排队是正确行为）
  - 延迟：在剩下的候选（未饱和，即新请求无需排队）里选延迟估计最低者

  这样**并发度为 1 的逐条调用也能避开慢 endpoint**——in-flight 在那里恒为 0，提供不了任何信息，只能靠延迟。并发升高后饱和过滤接手，回到容量感知。

  没有设 `concurrency_limit` 的 endpoint 无从判断饱和，改用 `cost = 延迟估计 × (in_flight + 1)`，让 in-flight 直接进入比较。
...
- **样本**来自成功的单条调用，取 `服务耗时 = result.latency − result.queue_time`。用结果自带的 `latency`（只覆盖真实请求）而不是在调用侧拿秒表计时，是因为秒表会把消息预处理（图片下载转 base64）也算进去，那不是 endpoint 的耗时；`queue_time` 是 semaphore 与 QPS 漏桶造成的本地排队，同样不算在 endpoint 头上。两类结果不计入，否则 endpoint 会显得比实际快而吸走流量：
  - **失败**：往往返回得更快，计入会让坏 endpoint 显得更优；处理失败是健康检查（`failure_threshold`）的职责
  - **缓存命中**：没有真实请求（`latency` 为 `None`），而且缓存跨 endpoint 共享，命中与否和是哪个 endpoint 无关
- **没有样本的 endpoint 按最乐观处理**，保证每个 endpoint 至少被探测一次；估计值相同时按负载率决胜，所以冷启动阶段的行为与纯容量感知完全一致。
- **闲置衰减**（`latency_decay_calls`，默认 10）：一个 endpoint 每被跳过 N 次选路，其延迟估计就衰减到约 37%，慢节点借此周期性地重新获得探测机会，恢复后无需人工干预即可回到轮换。慢 K 倍的节点大约每 `N·ln(K)` 次调用被探测一次——默认值下，慢 30 倍的节点约占 3% 的流量。调大则探测更稀疏（省掉慢请求的代价），调小则恢复更快。

  ```python
  pool = LLMClientPool(endpoints=[...], latency_decay_calls=40)
  ```

  衰减的单位是**选路次数**而不是秒，这一点很关键：endpoint 的快慢不会因为没人调用就改变，而探测的代价恰恰是按请求计的。按墙上时间衰减时，稀疏或交互式调用（每次调用时所有 endpoint 都已闲置很久）下衰减量由"谁最久没被选中"主导，延迟信息被完全抹掉，选路退化成轮询——慢节点照样吃到 `1/n` 的流量。用次数衡量后，密集批处理与每小时调一次的交互式使用有完全一致的探测频率。

  跳过次数多到估计值浮点下溢时（默认值下约 7000 次）退回冷启动，重新探测——那时信息确实已经完全过期。
